...
import logging
from Addmodule.block.HGBlock import HG_Block_Gate_test
...
class HG_Stage(nn.Module):
    def __init__(
        ...
        from Addmodule.down.DRFD import DRFD
        from Addmodule.down.RHDWT import RHDWT
        from Addmodule.down.Dirdown import DirDown
        from Addmodule.down.wave_down import Down_wt
        from Addmodule.down.test import WaveletGuidedDirDown, TheoreticallyOptimizedDirDown
        if downsample:
            if down == 'drfd':
                self.downsample = DRFD(in_chs, in_chs)
            elif down == 'dir':
                self.downsample = DirDown(in_chs)
            elif down == 'wt':
                self.downsample = Down_wt(in_chs, in_chs)
            elif down == 'rhdwt':
                self.downsample = RHDWT(in_chs, in_chs)
            elif down == 'test1':
                self.downsample = WaveletGuidedDirDown(in_chs, in_chs)
            elif down == 'test2':
                self.downsample = TheoreticallyOptimizedDirDown(in_chs, in_chs)
            else:
