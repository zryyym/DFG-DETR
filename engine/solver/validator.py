...
def compute_pr_data(pr_accumulator, iou_threshold=0.5):
    """计算PR曲线所需数据"""
    # 初始化数据结构
    ...
    # 收集所有数据
    for image_data in pr_accumulator.values():
        gt_boxes = image_data['gt']['boxes']
        gt_labels = image_data['gt']['labels']
        pred_boxes = image_data['pred']['boxes']
        pred_scores = image_data['pred']['scores']
        pred_labels = image_data['pred']['labels']

        for label in np.unique(np.concatenate([gt_labels, pred_labels])):
            # 收集当前类别的GT
            cls_gt_boxes = gt_boxes[gt_labels == label]
            all_gt[label].extend(cls_gt_boxes)
            ...
            all_pred[label]['boxes'].extend(cls_pred_boxes[sorted_indices])
            all_pred[label]['scores'].extend(cls_pred_scores[sorted_indices])

    # 初始化PR曲线数据
    recall_levels = 101
    px = np.linspace(0, 1, recall_levels)
    py = []
    ap = []
    class_names = {}

    # 计算每个类别的PR曲线
    for label, gt_boxes in all_gt.items():
        pred_boxes = np.array(all_pred[label]['boxes'])
        pred_scores = np.array(all_pred[label]['scores'])

        if len(pred_boxes) == 0:
            precision = np.zeros(recall_levels)
            ap.append(0.0)
            py.append(precision)
            class_names[label] = f"Class {label}"
            continue
        ...
        # 计算召回率和精确率
        recall = tp_cumsum / (len(gt_boxes) + 1e-16)
        precision = tp_cumsum / (tp_cumsum + fp_cumsum + 1e-16)

        # 插值到标准召回率点
        precision_interp = np.zeros(recall_levels)
        for r in range(recall_levels):
            recall_level = r / (recall_levels - 1)
            mask = recall >= recall_level
            if np.any(mask):
                precision_interp[r] = np.max(precision[mask])
            else:
                precision_interp[r] = 0
        ...
        for t in np.arange(0, 1.1, 0.1):
            mask = px >= t
            if np.any(mask):
                ap_value += np.max(precision_interp[mask]) / 11

        py.append(precision_interp)
        ap.append(ap_value)
        class_names[label] = f"Class {label}"

    # 转换为numpy数组
    py = np.array(py).T
    ap = np.array(ap).reshape(-1, 1)

    return px, py, ap, class_names
