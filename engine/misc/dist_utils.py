...
def setup_distributed(print_rank: int=0, print_method: str='builtin', seed: int=None, deterministic: bool=False):
    """
    env setup
    args:
        print_rank,
...
def setup_seed(seed: int, deterministic=False):
    ...
    if torch.backends.cudnn.is_available():
        if deterministic:
            ...
            torch.backends.cudnn.benchmark = False  # 必须关闭
            os.environ["CUBLAS_WORKSPACE_CONFIG"] = ":4096:8"
            # # 可选：启用严格确定性（PyTorch >= 1.8）
